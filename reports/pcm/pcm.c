...
static int seekChunk(FILE *fp, const char *id)
{
	for (;;) {
		...
		if (memcmp(tag, id, 4) == 0) return 1;
		if (fread(&n, 4, 1, fp) != 1) return 0;
		uint64_t remaining = (uint64_t)n + n % 2;
		unsigned char buffer[4096];
		while (remaining) {
			size_t count = remaining > sizeof(buffer) ? sizeof(buffer) : (size_t)remaining;
			if (fread(buffer, 1, count, fp) != count) return 0;
			remaining -= count;
		}
	}
}
...
static int writeData(FILE *fp, int n, const void *data)
{
	if (!fp) return (0);
	if (n < 0) return (0);
	if (fwrite(data, 1, n, fp) < (size_t)n) return (0);
	return (n);
}
...
{
	if (bit != 8 && bit != 16 && bit != 24 && bit != 32) return NULL;
	...
	// fmtチャンク部分の初期化
	int	bs = (bit/8)*ch;
	if (fs > UINT_MAX / bs || len > (unsigned int)(INT_MAX - 37) / bs) return NULL;
	...
	p->size = size;	// RIFFのパディングは標本データ長に含めない
	...
	for (int c = 0; c < (int)ch; c++) {
		p->val[c] = (double *)calloc(len, sizeof(double));
...
Wav *pcmRead(FILE *fp)
{
	...
	Fmt	fmt;
	if (!readData(fp, 16, &fmt)) return NULL;
	if (fmt.type != 1 || (fmt.ch != 1 && fmt.ch != 2) || fmt.fs == 0 ||
	    (fmt.bit != 8 && fmt.bit != 16 && fmt.bit != 24 && fmt.bit != 32) ||
	    fmt.bs != (fmt.bit / 8) * fmt.ch || fmt.fs > UINT_MAX / fmt.bs ||
	    fmt.dr != fmt.fs * fmt.bs) return NULL;
	...
	// dataチャンクの入力
	if (!seekChunk(fp, "data")) goto ERROR;
	p->size = readSize(fp);
	if (p->size == 0 || p->size > INT_MAX - 37 || p->size % p->fmt.bs) goto ERROR;
	...
	for (int c = 0; c < p->fmt.ch; c++) {
		p->val[c] = (double *)calloc(p->len, sizeof(double));
		if (!p->val[c]) goto ERROR;
	}
...
int pcmWrite(FILE *fp, const Wav *p)
{
	...
	// RIFFヘッダ
	writeData(fp, 4, "RIFF");
	unsigned int	size = p->size + 36 + p->size % 2;
...
double pcmMusicFreq(const char *s)
{
	...
	if (isdigit((unsigned char)*s) || *s == '.') {
		...
		return end != s && !*end && isfinite(value) && value > 0 ? value : 0.0;
	}
	...
	int	oct = 4;
	if (*s) {
		if (s[0] < '0' || s[0] > '9' || s[1] != '\0') return 0.0;
		oct = s[0] - '0';
	}
